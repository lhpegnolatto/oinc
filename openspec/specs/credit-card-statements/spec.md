...
A credit card's charges roll up into periodic statements — a closed cycle with a total
and a due date — and a user needs to pay off that statement from a wallet. This
capability covers computing a card's statement cycle boundaries and closed-cycle total
from its existing `statementCloseDay`/`dueDay` (no new fields on the card or its
charges), displaying that total and due date alongside the card's running `balance`,
and recording/deleting payments against a card from a wallet the user owns.
